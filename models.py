...
class Attention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.):
        ...
        self.scale = pow(self.dim_head, 0.5)
    ...
    def forward(self, x, context=None, kv_include_self=False):
        ...
        # TODO: attention
        # calculating the key, query, and value matrices
        Query = self.Q(x)
        Key = self.K(context)
        ...
        AttentionScores = self.softmax(AttentionScores)
        AttentionScores = self.dropout(AttentionScores)

        # multiplying attention scores by values
        MultiHead_out = torch.einsum("bhnk,bhkd->bhnd", AttentionScores, MultiHead_V)
